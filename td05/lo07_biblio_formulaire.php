...
function form_input_text($label, $name, $size, $value) {
    ...
    echo ("<div class='form-group w-50' style='margin-bottom: 10px'>");
...
function form_select($label, $name, $multiple, $size, $liste) {
    echo ("<div class='form-group w-50' style='margin-bottom: 10px'>");
    echo (" <label for='$label' class='fw-bold'>$label</label><br>");
    echo (" <select class='form-select' name='$name' $multiple size='$size'>");
...
function form_input_submit($value) {
    echo (" <button type='submit' class='btn btn-success'>$value</button>");
}
